...
namespace utk {
    static std::string getExt(std::string f, std::string default_value) {
    ...
    static std::string getBasename(std::string f) {
    ...
    template<typename T>
    static bool isFloatingType() {
        return typeid(T) == typeid(float) ||
               typeid(T) == typeid(double);
    }

    static bool *toBitList(unsigned long long int a, unsigned int size) {
